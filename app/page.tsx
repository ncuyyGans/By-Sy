...
export default async function Home() {
  ...
  return <main className="site-shell"><SiteHeader settings={settings} /><section className="profile-hero"><p className="intro-label">Hello, I&apos;m</p><h1>{settings.name}</h1><div className="profile-copy"><p className="role">{settings.role}</p><p>{settings.intro}</p></div><div className="profile-meta"><span>{settings.location}</span><div>{socials.map(([label, url]) => <a key={label} href={url} target="_blank" rel="noreferrer">{label}</a>)}<a href={`mailto:${settings.email}`}>Email</a></div></div></section><section className="home-panels" aria-label="Tentang dan karya"><Link className="home-panel" href="/about"><span className="eyebrow">01 / About</span><strong>Kenalan lebih dekat →</strong><span>Cerita singkat tentang siapa aku dan apa yang sedang kubangun.</span></Link><Link className="home-panel" href="/projects"><span className="eyebrow">02 / Projects</span><strong>Lihat karya →</strong><span>Project, eksperimen, dan jejak kerja yang ingin kubagikan.</span></Link></section><section className="writing-section"><div className="section-heading"><h2>Recent writing</h2><Link href="/blog">View all</Link></div>{posts.length ? <div className="post-list">{posts.slice(0, 5).map((post) => <PostRow key={post.id} post={post} />)}</div> : <div className="empty-state"><p>Belum ada tulisan yang diterbitkan.</p></div>}</section><footer className="site-footer"><p>{settings.footer}</p><p>© {new Date().getFullYear()}</p></footer></main>;
}
